Application.$controller("BoxOAuthController", ["$scope", "Utils", function($scope, Utils) {
    "use strict";
    var LOGIN_MODE = {
            'AUTOMATIC': "auto"
        },
    ...
    $scope.OauthcallbackGetAccessTokenonSuccess = function(variable, data) {
        var callbackParams;
        if (data) {
            ...
            $scope.Variables.OauthcallbackGetLoginURL.setInput('state', callbackParams);
            $scope.Variables.OauthcallbackGetLoginURL.invoke();
        }
    };


    $scope.OauthcallbackGetLoginURLonSuccess = function(variable, data) {
        $scope.loginurl = data;
